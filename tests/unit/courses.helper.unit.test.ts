...
describe("Get some Courses", () => {
  describe("Fetching some courses", () => {
    it("getAllCourses should return a json of all courses", () => {
      const courses: { [key: string]: Course } = getCourses([
        "I&CSCI46",
        "ARTHIS134E",
      ]);
      expect(courses).not.toBeNull();
      expect(typeof courses).toBe("object");
      expect(courses["I&CSCI46"]).toMatchObject({
        number: "46",
        department: "I&C SCI",
      });
      expect(courses["ARTHIS134E"]).toMatchObject({
        number: "134E",
        department: "ART HIS",
      });
    });
  ...
  describe("Fetching course", () => {
    it("getCourse should return a json of the course requested", () => {
      ...
      expect(course).toMatchObject({
        ...
        professor_history: expect.arrayContaining([
          "sgagomas",
          "klefstad",
          "pattis",
          "thornton",
      ...
      expect(course["prerequisite_for"]).toEqual(
